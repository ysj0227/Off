...
- (void)scrollViewSubviews:(UIScrollView*)scroll {
    ...
    if (titles && _headStyle != SegmentHeadStyleSlide) {
        ...
        if (_fontScale != 1) {
            if (!self.selectedBold) {
                curBtn.titleLabel.font = [UIFont systemFontOfSize:_fontSize*_fontScale];
            }else{
                curBtn.titleLabel.font = [UIFont boldSystemFontOfSize:_fontSize*_fontScale];
            }
...
- (void)changeBtnFrom:(NSInteger)from to:(NSInteger)to {
    ...
    if (_fontScale) {
        if(self.selectedBold) {
            select_btn.titleLabel.font = [UIFont boldSystemFontOfSize:_fontSize*_fontScale];
        }else{
...
- (void)changePointScale:(CGFloat)scale {
    ...
    switch (_headStyle) {
        ...
        case SegmentHeadStyleArrow:
        {
            ...
            //颜色变化
            if (self.gradualChangeTitleColor) {
...
- (void)fontChangeCurBtn:(UIButton *)curBtn nextBtn:(UIButton *)nextBtn changeScale:(CGFloat)changeScale{
    ...
    if (self.selectedBold) { // boldSystemFontOfSize
        curBtn.titleLabel.font = [UIFont boldSystemFontOfSize:cur_font];
    }else{
