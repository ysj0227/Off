...
@objc open class AORangeSlider: UIControl {
    ...
    private let systemBallLength: CGFloat = 20.0
    ...
    @objc @IBInspectable open var trackBackgroundColor: UIColor = kAppColor_999999 {
        didSet {
            self.trackBackgroundImage = AORangeSlider.getImage(color: self.trackBackgroundColor, size: CGSize(width: 1, height: 3))
        }
    }
    ...
    func trackImageForCurrentValues() -> UIImage? {
        if lowValue <= highValue {
            if trackImage != nil {
                ...
                return AORangeSlider.getImage(color: trackColor!, size: CGSize(width: 1, height: 3))
            }else {
                return AORangeSlider.getImage(color: #colorLiteral(red: 0.2745098039, green: 0.7647058824, blue: 0.7607843137, alpha: 1), size: CGSize(width: 1, height: 3))
            }
        } else {
            if trackCrossedImage != nil {
                return trackCrossedImage
            }else if(trackCrossedColor != nil) {
                return AORangeSlider.getImage(color: trackCrossedColor!, size: CGSize(width: 1, height: 3))
            }else {
                return AORangeSlider.getImage(color: .red, size: CGSize(width: 1, height: 3))
            }
    ...
    private func configureViews() {
        ...
        trackBackgroundImage = AORangeSlider.getImage(color: #colorLiteral(red: 0.7333333333, green: 0.7333333333, blue: 0.7333333333, alpha: 1), size: CGSize(width: 1, height: 3))
