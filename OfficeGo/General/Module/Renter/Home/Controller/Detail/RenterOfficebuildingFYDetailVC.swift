...
class RenterOfficebuildingFYDetailVC: BaseTableViewController {
    ...
    func shareVc() {
        ...
        shareVC.shareUrl = "\(SSAPI.SSH5Host)\(SSDelegateURL.h5BuildingFYDetailShareUrl)?isShare=\(UserTool.shared.user_channel)&buildingId=\(buildingFYDetailViewModel?.houseViewModel?.buildingId ?? 0)&houseId=\(buildingFYDetailModel?.house?.id ?? 0)"
...
///头部图片点击展示代理
extension RenterOfficebuildingFYDetailVC: RenterDetailSourceViewImgScanDelegate{
    func vrClick() {
