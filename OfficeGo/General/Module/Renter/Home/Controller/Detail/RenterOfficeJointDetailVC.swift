...
class RenterOfficeJointDetailVC: BaseGroupTableViewController, WMPlayerDelegate {
        ...
        if let params = shaiXuanParams {
            if let seats = params["seats"] {
                ...
                if str?.count ?? 0 > 0 {
                    ...
                    shaixuanAreaSeatsString = str?.replacingOccurrences(of: String(format: ",%.0f", noLimitMaxNum), with: "~不限")
