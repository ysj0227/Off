...
class RenterOfficeJointDetailVC: BaseTableViewController, WMPlayerDelegate {
    ...
    func shareVc() {
        ...
        shareVC.shareUrl = "\(SSDelegateURL.h5BuildingDetailShareUrl)?buildingId=\(buildingDetailViewModel?.buildingViewModel?.buildingId ?? 0)"
    ...
    func scrollToFY() {
        ...
        if dataSourceArr.count > 2 {
            if dataSource.count > 0 {
                tableView.scrollToRow(at: IndexPath.init(row: 0, section: 2), at: UITableView.ScrollPosition.top, animated: true)
            }
