...
class PhoneExchangeMessage: RCMessageContent, NSCoding {
    ...
    override class func getObjectName() -> String! {
        return AppKey.PhoneExchangeMessageTypeIdentifier
    }
}
...
//            let image = RCKitUtility.imageNamed("chat_to_bg_normal", ofBundle: "RongCloud.bundle")
    ...
    private class func getTextLabelSize(_ message: PhoneExchangeMessage, messageDirection: RCMessageDirection) -> CGSize {
        if messageDirection == RCMessageDirection.MessageDirection_RECEIVE {
            message.content = "对方想获取你的手机号～"
        }else {
            message.content = "你像对方发送了请求手机号～"
        }
...
//            return CGSize(width: textRect.size.width + 5, height: textRect.size.height + 5)
    ...
    public class func getBubbleBackgroundViewSize(_ message: PhoneExchangeMessage, messageDirection: RCMessageDirection) -> CGSize {
        
        let textLabelSize = PhoneExchangeMessageCell.getTextLabelSize(message, messageDirection: messageDirection)
        return PhoneExchangeMessageCell.getBubbleSize(textLabelSize)
