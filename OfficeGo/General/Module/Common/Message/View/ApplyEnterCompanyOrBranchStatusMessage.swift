//
//  ApplyEnterCompanyOrBranchStatusMessage.swift
//  OfficeGo
//
...
class ApplyEnterCompanyOrBranchStatusMessage: RCMessageContent, NSCoding {
    ...
    // 根据参数创建消息对象
    class func messageWithContent(content: String, isAgree: Bool) -> ApplyEnterCompanyOrBranchStatusMessage {
        let testMessage = ApplyEnterCompanyOrBranchStatusMessage()
    ...
    override class func getObjectName() -> String! {
        return AppKey.ApplyEnterCompanyOrBranchStatusMessageTypeIdentifier
    }
}


class ApplyEnterCompanyOrBranchStatusMessageCell: RCMessageBaseCell {
    ...
    override class func size(for model: RCMessageModel!, withCollectionViewWidth collectionViewWidth: CGFloat, referenceExtraHeight extraHeight: CGFloat) -> CGSize {
        ...
        let message = model.content as? ApplyEnterCompanyOrBranchStatusMessage ?? ApplyEnterCompanyOrBranchStatusMessage.messageWithContent(content: "", isAgree: false)
        let size = ApplyEnterCompanyOrBranchStatusMessageCell.getTextLabelSize(message, messageDirection: model.messageDirection)
    ...
    private func setContent(_ model: RCMessageModel!) {
        let testMessage = model.content as? ApplyEnterCompanyOrBranchStatusMessage
        //接收 -
        if RCMessageDirection.MessageDirection_RECEIVE == messageDirection {
            if testMessage?.isAgree == true {
                testMessage?.content = "已加入"
            }else {
                testMessage?.content = "对方拒绝了你的申请"
            }
        }else {
            if testMessage?.isAgree == true {
    ...
    private func setAutoLayout() {
        ...
        let textLabelSize = ApplyEnterCompanyOrBranchStatusMessageCell.getTextLabelSize(testMessage ?? ApplyEnterCompanyOrBranchStatusMessage.messageWithContent(content: "", isAgree: testMessage?.isAgree ?? false), messageDirection: messageDirection)
