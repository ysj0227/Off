...
extension NewIdentifyViewController {
    ...
    func selectZLAgentPicker() {
        var imgArr = [BannerModel]()
        zlAgentImagePickTool.cl_setupImagePickerWith(MaxImagesCount: ownerMaxZLAgentNumber_5 - uploadPicModelBuChongArr.count) {[weak self] (asset,cutImage) in
            // 内部提供的方法可以异步获取图片，同步获取的话时间比较长，不建议！，如果是iCloud中的照片就直接从icloud中下载，下载完成后返回图片,同时也提供了下载失败的方法
            CLImagePickerTool.convertAssetArrToOriginImage(assetArr: asset, scale: 0.1, successClouse: {[weak self] (image,assetItem) in
                let img = image.resizeMax1500Image()
            ...
            self?.uploadPicModelBuChongArr.append(contentsOf: imgArr)
            self?.loadCollectionData()
        }
    }
    
    ///营业执照
    func selectYingyePicker() {
        var imgArr = [BannerModel]()
        zlAgentImagePickTool.cl_setupImagePickerWith(MaxImagesCount: ownerMaxZLAgentNumber_5 - uploadPicModelBuChongArr.count) {[weak self] (asset,cutImage) in
            // 内部提供的方法可以异步获取图片，同步获取的话时间比较长，不建议！，如果是iCloud中的照片就直接从icloud中下载，下载完成后返回图片,同时也提供了下载失败的方法
            CLImagePickerTool.convertAssetArrToOriginImage(assetArr: asset, scale: 0.1, successClouse: {[weak self] (image,assetItem) in
            ...
            self?.uploadPicModelYingyeArr.append(contentsOf: imgArr)
            self?.loadCollectionData()
        }
    }
    ...
    func request_deleteZLAgentImgApp(index: Int) {
        ...
        params["id"] = uploadPicModelBuChongArr[index].id as AnyObject?
